...
function warn_unsuitable_java {
    ...
    echo "Connect Tunnel needs Java $XG_JAVA_REQD or" \
         "newer; your Java$XG_CURRJVER appears unsuitable."
    echo -n "Try to run anyway? (y/n) "
    read -n1 overridebadjava
    echo
    if [ "$overridebadjava" != "y" -a "$overridebadjava" != "Y" ]; then
        return 1
    fi
...
java $XG_DEBUG $XG_SCALE -jar "$xg_inst_dir/$XG_UI_ARCHIVE" "$@"
